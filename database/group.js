	...
	//    if (!isWelcome) return
		try {
         ...
         //    if (!welkom.includes(chat.jid)) return
              ...
              pebz.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `Welcome Message By * *`, buttons: [{buttonId: `.selamatdatang`,buttonText:{displayText: 'WELCOME KAK'},type:1},{buttonId: `.rules`,buttonText:{displayText:'RULES BOT'},type:1},{buttonId:`.infogrouup`,buttonText:{displayText:'INFO GROUP'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		}
        ...
        //    if (!welkom.includes(chat.jid)) return
               ...
               buff = await getBuffer(pic)
               pebz.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Left Message By * *`, buttons: [{buttonId: `.bay`,buttonText:{displayText: 'BYE KAK👋'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
		} catch (e) {
